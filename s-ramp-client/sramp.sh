...
then
    ...
    echo "sramp.sh brms [pkg_in_brms] [tag] [baseUrl] [userId] [password] [srampUrl].....obtains 'pkg_in_brms' from brms/drools and uploads to s-ramp"
    exit 0
fi
...
then
    ...
    java -cp target/s-ramp-client-0.0.3-SNAPSHOT.jar:$(echo target/lib/*.jar | tr ' ' ':') org.overlord.sramp.client.brms.BrmsPkgToSramp $2 $3 $4 $5 $6 $7
    exit 0
fi
