 ...
 * @author Brett Meyer
 */
@CommandDefinition(name = "deleteContent",
		description = "The \"deleteContent\" command removes an artifact's content from the Artificer repository.  The artifact can be identified either by its unique Artificer uuid or else by an index into the most recent Feed.  Additionally, the currently active artifact can have its content deleted by omitting the uuid and feed options.\n")
public class DeleteContentCommand extends AbstractCommand {
	...
	protected CommandResult doExecute(CommandInvocation commandInvocation) throws Exception {
		...
		try {
			client.deleteArtifactContent(artifactUuid, artifactType);
			commandInvocation.getShell().out().println(Messages.i18n.format("DeleteContent.Success", artifactName)); //$NON-NLS-1$
		} catch (Exception e) {
			commandInvocation.getShell().out().println(Messages.i18n.format("DeleteContent.Failure")); //$NON-NLS-1$
