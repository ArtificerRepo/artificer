...
/**
 * Validates that the File input is a directory. in case is not it throws an
 * exception.
 *
 * @author David Virgil Naranjo
 */
public class FileDirectoryValidator implements OptionValidator<ValidatorInvocation<File>> {

    /**
     * Instantiates a new file directory validator.
     */
    public FileDirectoryValidator() {
    ...
    public void validate(ValidatorInvocation<File> validatorInvocation) throws OptionValidatorException {
        try {
            checkIsDirectory(validatorInvocation.getValue());
     ...
     */
    public static void checkIsDirectory(File path) throws InvalidCommandArgumentException {
        if (path.isFile()) {
            String fileName = "";
            ...
            throw new InvalidCommandArgumentException(Messages.i18n.format(
                    "Validator.OutputFileExists", fileName)); //$NON-NLS-1$
        }
    }
}