 ...
 */
public class TabCompleter implements CompleterInvocationProvider<CompleterInvocation> {
     ...
     * 
     * @see
     * org.jboss.aesh.console.command.completer.CompleterInvocationProvider#
     * enhanceCompleterInvocation
     * (org.jboss.aesh.console.command.completer.CompleterInvocation)
     */
    @Override
    public CompleterInvocation enhanceCompleterInvocation(CompleterInvocation completeOperation) {
        String buffer = completeOperation.getGivenCompleteValue();
        // Case 1 - nothing has been typed yet - show all command namespaces
        if (buffer.trim().length() == 0) {
            for (String ns : factory.getNamespaces()) {
                completeOperation.addCompleterValue(ns + ":");
            }
        }
        ...
        if (!buffer.contains(":") && !buffer.contains(" ")) { //$NON-NLS-1$ //$NON-NLS-2$
            ...
            if (completeOperation.getCompleterValues().isEmpty()) {
                for (QName cmdName : factory.getCommandNames("s-ramp")) { //$NON-NLS-1$
                    if (cmdName.getLocalPart().startsWith(buffer)) {
                        completeOperation.addCompleterValue("s-ramp:" + cmdName.getLocalPart() + " "); //$NON-NLS-1$ //$NON-NLS-2$
                    }
                }
            }
        }
        ...
        if (buffer.endsWith(":") && !buffer.contains(" ")) { //$NON-NLS-1$ //$NON-NLS-2$
            ...
            for (QName cmdName : factory.getCommandNames(ns)) {
                completeOperation.addCompleterValue(cmdName.getLocalPart() + " "); //$NON-NLS-1$
            }
        }
