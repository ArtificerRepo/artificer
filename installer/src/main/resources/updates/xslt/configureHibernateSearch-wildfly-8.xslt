...
<xsl:template match="*[name()='module']/*[name()='dependencies']">
  <xsl:choose>
    <!-- If it already has the tika dependency, then just copy everything as-is -->
    <xsl:when test="*[name()='module'][@name='org.apache.tika']">
      <xsl:copy>
        <xsl:apply-templates select="@*|node()|text()" />
      </xsl:copy>
    </xsl:when>
    <!-- Otherwise, we need to insert the tika dependency -->
    <xsl:otherwise>
      ...
      <xsl:element name="dependencies" namespace="{$currentNS}">
        ...
        <xsl:element name="module" namespace="{$currentNS}">
          <xsl:attribute name="name">org.apache.tika</xsl:attribute>
          <xsl:attribute name="slot">1.4</xsl:attribute>
        </xsl:element>
      </xsl:element>
    </xsl:otherwise>
  </xsl:choose>
