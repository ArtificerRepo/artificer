            ...
            if (tokens.length >= 4) {
                ...
                if (file_name.contains(".")) {
                    ...
                    try {
                        ...
                        logger.info(Messages.i18n.format("maven.servlet.artifact.content.get.exception",
                                groupId, artifactId, version, file_name));
                    } finally {
                        if (artifact != null && artifact.getContent() != null) {
                            artifact.getContent().close();
                        }
                    }
                }
                ...
                if (StringUtils.isNotBlank(url) && !url.equals("/")) {
                    ...
                    if (urlTokens.length > 1) {
                        for (int i = 0; i < urlTokens.length - 1; i++) {
                            parentPath += urlTokens[i] + "/";
                        }
                    }
                    parentPath = "/" + parentPath;
                    req.setAttribute("parentPath", parentPath);
                }
                req.setAttribute("relativePath", url);
